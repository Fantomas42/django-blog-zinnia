...
urlpatterns = patterns('django.views.generic.list_detail',
                       url(r'^$', 'object_list',
                           entry_conf_list, 'zinnia_entry_archive_index'),
                       url(r'^page/(?P<page>\d)/$', 'object_list',
                           entry_conf_list, 'zinnia_entry_archive_index_paginated'),
                       )

urlpatterns += patterns('django.views.generic.date_based',
                        url(r'^(?P<year>\d{4})/$', 'archive_year',
                            entry_conf_year, 'zinnia_entry_archive_year'),
                        url(r'^(?P<year>\d{4})/(?P<month>\w+)/$', 'archive_month',
                            entry_conf, 'zinnia_entry_archive_month'),
                        url(r'^(?P<year>\d{4})/(?P<month>\w+)/(?P<day>\d{2})/$', 'archive_day',
                            entry_conf, 'zinnia_entry_archive_day'),
                        url(r'^(?P<year>\d{4})/(?P<month>\w+)/(?P<day>\d{2})/(?P<slug>[-\w]+)/$', 'object_detail',
                            entry_conf_detail, 'zinnia_entry_detail'),
                        )
